...
“5.3.1.1 创建内部帐户”是认证器应用程序的示例，“5.3.1.2 使用内部帐户”是请求应用程序的示例。 在 JSSEC 网站上分发的示例代码集中，每个代码集都对应账户管理器的认证器和用户。

#### 5.3.1.1 创建内部账户

以下是认证器应用程序的示例代码，它使账户管理器能够使用内部帐户。 在此应用程序中没有可以从主屏幕启动的活动。 请注意，它间接通过账户管理器，从另一个示例代码“5.3.1.2 使用内部帐户”调用。

要点：

1.  提供认证器的服务必须是私有的。
2.  登录界面的活动必须在验证器应用程序中实现。
3.  登录界面的活动必须实现为公共活动。
...
public class WebService {
    ...
    */
    ...
    private String getAuthToken(String username, String password) {
        // In fact, get the value which uniqueness and impossibility of speculation are guaranteed by the
        server,
